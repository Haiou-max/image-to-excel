...
3. 客户意向价 - 【必填字段】标准格式下方一行最后是"X元"，这个字段一定存在。从符号前面提取数字，小数点务必保留。【强制纠错规则】手写价格中小数点可能很小、很淡、或被两个数字连笔掩盖，必须逐位仔细辨认。特别注意：如果识别出的数字是两位数（如39、55、510），而且看起来偏大，很可能漏了小数点。常见误读：3.9→39、5.5→55、10.5→105。遇到这种情况要反思是否在两位数中间漏了小数点。外贸产品单价通常不超过20元，如果识别超过50的数字，必须检查是否漏了小数点
