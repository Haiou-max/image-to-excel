...
3. 客户意向价 - 【必填字段】标准格式下方一行最后是"X¥"，这个字段一定存在。从符号前面提取数字，小数点务必保留。【强制纠错规则】手写价格中小数点可能很小、很淡、或被两个数字连笔掩盖，必须逐位仔细辨认。特别注意：如果识别出的数字是两位数（如39、55、510），而且看起来偏大，很可能漏了小数点。常见误读：3.9→39、5.5→55、10.5→105。遇到这种情况要反思是否在两位数中间漏了小数点。外贸产品单价通常不超过200，如果识别超过200的数字，必须检查是否漏了小数点
4. 做货要求一 - 只填尺寸/规格相关的纯数字信息（7×9、15.5×20.5、7cm 等）。【强制规则】"X付""X组"等非尺寸词汇不要放这里。如果完全没有纯数字尺寸信息，此字段留空
5. 箱数 - 【必填字段】标准格式下方一行开头是"X件 × ..."，这个"件"字一般存在，箱数一定能找到。【三种来源，按优先级】(a)"件"前的数字：找到"件"字位置，提取其左边的数字（如"20件"→20）。(b)"箱"前的数字：若文字中出现"箱"字（如"20箱"、"5箱"），则"箱"前面的数字直接作为箱数。(c)裸数字：若该位置没有"件"也没有"箱"字，只有一个数字（如"20 × 300 PCS × 2¥"），则这个数字直接作为箱数。【三步提取法】第一步：在第二行中间部分仔细找到"件"/"箱"字的位置。第二步：找到后将其位置标记为分割点，排除字本身。第三步：提取左边的数字作为箱数；若没有"件""箱"字，则取该位置的裸数字。【强制纠错】如果看到"×"符号后面出现了数字，说明已经超过分割点了，要回头重新定位。不能轻易标[待确认]。
